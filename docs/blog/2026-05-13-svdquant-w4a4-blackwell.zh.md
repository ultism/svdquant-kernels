...
*面向写 CuTe DSL 的 SM_100/SM_103 内核开发者的教学博文。讲清楚本仓库的
`gemm_w4a4` 怎么从 1-CTA 的 CUTLASS 例程直译版重构成 FA4 衍生的 2-CTA
持久化内核，每个 Blackwell 原语在其中扮演什么角色，以及哪一行账目失算
把 +198 % 的 TF 藏在了"看上去能跑"的 smoke 后面。*
...
上卡在 ~27 % MFU；想把它升到 2-CTA 走 `cta_group=TWO` 的 Phase-1 尝试
...
MFU；在可比的 shape 上 fp16 全数 4/4 领先 nunchaku，bf16 3/4 领先。

整个项目里单行改动 ROI 最高的一处：把 2-CTA 下每 CTA 的 LU SMEM 估算
减半。原本 `_compute_stages` 读到的 `lora_smem_bytes` 在 LU 那一项算大了
2×，预算求解器悄悄把 `num_ab_stage` 从 4 裁到 2。症状是：**没有症状** ——
trace 通、kernel 跑、数值对、看上去就是"有点慢"。改法：多写一个
`// self.cta_group_size`。生产 shape 上的墙钟：**566 TF → 1685 TF（+198 %）**、
**4.2 % → 16.9 % MFU**。同样 launch 配置下的 ncu A/B：Duration −31.2 %、
SM Throughput +11.99 pp、SM Active Cycles −36.3 %。提交 `7296e90`；完整
数据在 §7。
...
- **vLLM 在线性边界上的 drop-in**。输入进、输出出，下一个算子是什么我们
  不知道。nunchaku 的 `fuse_glu` 捷径（把前一层 SwiGLU 的 gate 吞进
  quantize 步）和 `fused_gelu_mlp` 路径（下一层的 `smooth_factor` 倒灌进
  本层 epilogue）一律不做 —— 那两类都需要框架层的融合。下一层 NVFP4
  quantize 的 epilogue 想法（v3）在 `docs/architecture.md` 里因为同样的
  理由被砍掉。
- **只做 SM_100 / SM_103**。消费级 Blackwell SM_120a/121a 由 nunchaku
  覆盖（架构矩阵见 `tmp/nunchaku/setup.py:41-64`），本仓库的存在意义就是
...
陷阱在 `docs/gotchas/cute_dsl.md:90-151`：每 CTA 在 cluster 里的 M 位置
存在 **index `[0]`（V）**，不是 `[2]`（N）。读 `[2]` 看上去顺眼，因为
**问题**那一侧的 N 轴在那 —— 但 `[2]` 是 cluster 侧的 N 切分；M-major
的 pair 里两个 CTA 共享 `[2] = 0`，用它去区分 CTA 会把所有 tile 都派给
leader，持久化 tile 解码当场崩。非持久版本的同名变量算在
`cute_kernels/gemm_w4a4/kernel.py:956-960`，用的是 `bidx // atom_thr_size`。
...
`tmp/flash-attention/flash_attn/cute/`）在另一个算子上已经解决了上面
...
3. **TV-layout match**。主 NVFP4 atom 和 LoRA fp16/bf16 atom 把 per-CTA
   ...
   引用；原始校验通过 `tmp/verify_tmem_layout.py` 在 bring-up 期跑，
...
`tmp/nunchaku/setup.py:41-64`）这条上 gated，所以我们在 B200 上没法
...
（出处：`docs/gpu.md:314-319`。）**fp16：4/4 shape 领先。bf16：3/4
shape 领先**。唯一 nunchaku 还领先的 bf16 shape（M=4352 K=15360 N=3840，
−3.2 pp）是 `docs/gpu.md:79-103` 说的"bf16 hand-PTX vs DSL MLIR lowering"
不对称：nunchaku 的 MMA 是 inline PTX（`mma_earlycuda.cuh`），fp16 和
bf16 是两套分别手调的 PTX，register packing / 累加器精度不同。我们走
一个 `tcgen05` atom + `ab_dtype` 替换 —— 两种 dtype 走同一份 MLIR。要
补上 bf16 上最后那 3 pp 大概率得下到 inline PTX，超出范围。
...
**致谢**。感谢 Verda（启智社区）提供 counter 不受限的 B200 镜像 ——
LU SMEM 修复在受限计数器的主机上只会读作墙钟噪声，C1 的机制分析也
literally 需要计数器。感谢 Tri Dao 的 Flash-Attention 4，warp-spec
